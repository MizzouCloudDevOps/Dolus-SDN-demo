...
echo -e ${LIGHTBLUE}"################################################################"
echo "# DDoS Controller Installation and Configuration Script  #"
echo "#                                                              #"
...
apt-get -y install nano wget apache2 python-setuptools python-dev build-essential libcurl4-openssl-dev opam curl apt dpkg mininet m4 zlib1g-dev python-pip libmysqlclient-dev php libapache2-mod-php libssl-dev || checkErr "Package installation"
...
echo -e "${BLUE}\nConfiguring Opam...${NC}"
mkdir /opam
opam init -y --root /opam || checkErr "Opam configuration"
opam switch create 4.06.0 --root /opam || checkErr "Opam configuration"
opam switch 4.06.0  --root /opam|| checkErr "Opam configuration" || checkErr "Opam configuration"
opam switch  --root /opam|| checkErr "Opam configuration" || checkErr "Opam configuration"
eval `opam config env --root=/opam` || checkErr "Opam configuration"
echo 'eval `opam config env --root=/opam`' >> ~/.profile || checkErr "Opam configuration"
...
opam pin add frenetic -y https://github.com/wangso/frenetic.git --root /opam || checkErr "Frenetic installation"
...
apt-get -y install ufw || checkErr "ufw installation issue"
